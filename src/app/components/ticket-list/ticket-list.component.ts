...
export class TicketListComponent implements OnInit {
  ...
  filter(): void {
    console.log(' this.assinadoToMe --> ',this.assignedToMe);
    this.page = 0;
    this.count = 5;
    this.ticketService.findByParams(this.page, this.count, this.assignedToMe, this.ticketFilter)
      .subscribe((responseApi: ResponseApi) => {
        this.ticketFilter.titulo = this.ticketFilter.titulo == 'uniformed' ? "" : this.ticketFilter.titulo;
  ...
  delete(id:string){
    this.dialogService.confirm('Você realmente deseja excluir o Ticket?')
